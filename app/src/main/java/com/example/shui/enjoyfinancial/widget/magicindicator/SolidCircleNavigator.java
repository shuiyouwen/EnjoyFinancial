 ...
 */
public class SolidCircleNavigator extends View implements IPagerNavigator {
    private int mRadius;
    ...
    public SolidCircleNavigator(Context context) {
        super(context);
        init(context);
    }
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        switch (mode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                result = mTotalCount * mRadius * 2 + (mTotalCount - 1) * mCircleSpacing + getPaddingLeft() + getPaddingRight();
                break;
            default:
                break;
        }
    ...
    private void drawCircles(Canvas canvas) {
        ...
        for (int i = 0, j = mCirclePoints.size(); i < j; i++) {
            ...
            canvas.drawCircle(pointF.x, pointF.y, mRadius, mNormalPaint);
        }
    }
    ...
    private void drawIndicator(Canvas canvas) {
        ...
        if (mCirclePoints.size() > 0) {
            canvas.drawCircle(mIndicatorX, (int) (getHeight() / 2.0f + 0.5f), mRadius, mSelectPaint);
        }
    }
    ...
    private void prepareCirclePoints() {
        ...
        if (mTotalCount > 0) {
            ...
            int startX = mRadius + getPaddingLeft();
