...
public class StatusBarUtil {
     ...
     * @param isFontBlack    状态栏文字是否黑色，默认设置为黑色
     */
    public static void stateBarSetting(Activity activity, View needOffSetView, boolean isFontBlack) {
        if (isFontBlack) {
            stateBarSetting(activity, needOffSetView);
        } else {
            setTranslucentForImageViewInFragment(activity, 0, needOffSetView, false);
        }
    }
    ...
    public static void setColor(Activity activity, @ColorInt int color) {
        setColor(activity, color, DEFAULT_STATUS_BAR_ALPHA, false);
    }
    ...
    public static void setColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha, boolean mIsFontBlack) {
        if (mIsFontBlack) {
            String carrier = Build.MANUFACTURER;
            if (TextUtils.equals(carrier.toUpperCase(), "XIAOMI")) {
                setMiuiStatusBarDarkMode(activity, true);
            } else if (TextUtils.equals(carrier.toUpperCase(), "MEIZU")) {
    ...
    public static void setColorNoTranslucent(Activity activity, @ColorInt int color) {
        setColor(activity, color, 0, false);
    }
     ...
     * @param statusBarAlpha 状态栏透明度
     * @param needOffsetView 需要向下偏移的 View
     * @param isFontBlack    文字是否黑色
     */
    public static void setTranslucentForImageView(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha,
                                                  View needOffsetView, boolean isFontBlack) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        setTransparentForWindow(activity, isFontBlack);
        addTranslucentView(activity, statusBarAlpha);
        if (needOffsetView != null) {
            Object haveSetOffset = needOffsetView.getTag(TAG_KEY_HAVE_SET_OFFSET);
            if (haveSetOffset != null && (Boolean) haveSetOffset) {
    ...
    public static void setTranslucentForImageViewInFragment(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha,
                                                            View needOffsetView,boolean isFontBlack) {
        setTranslucentForImageView(activity, statusBarAlpha, needOffsetView, isFontBlack);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            clearPreviousSetting(activity);
        }
    }
     ...
     * 设置透明
     */
    private static void setTransparentForWindow(Activity activity, boolean isFontBlack) {
        String carrier = Build.MANUFACTURER;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().setStatusBarColor(Color.TRANSPARENT);
            if (isFontBlack && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !TextUtils.equals(carrier.toUpperCase(), "XIAOMI") && !TextUtils.equals(carrier.toUpperCase(), "MEIZU")) {
                activity.getWindow()
                        .getDecorView()
        ...
        if (isFontBlack && TextUtils.equals(carrier.toUpperCase(), "XIAOMI")) {
            setMiuiStatusBarDarkMode(activity, true);
        } else if (isFontBlack && TextUtils.equals(carrier.toUpperCase(), "MEIZU")) {
            setMeizuStatusBarDarkIcon(activity, true);
        }
    }
